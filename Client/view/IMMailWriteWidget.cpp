...
void IMMailWriteWidget::onClickBtnSend()
{
    ...
    {
        QMessageBox::about(NULL, QStringLiteral("提示"), QStringLiteral("收件人不能为空！"));
        return;
    }
    if (m_leReceiverAddress->text().compare(m_myID) == 0)
    {
        QMessageBox::about(NULL, QStringLiteral("提示"), QStringLiteral("不能给自己发送邮件！"));
        return;
    }
    if (m_leTheme->text().isEmpty())
    {
        QMessageBox::about(NULL, QStringLiteral("提示"), QStringLiteral("信件主题不能为空"));

        return;
    }
    if (m_textInput->toPlainText().isEmpty())
    {
        QMessageBox::about(NULL, QStringLiteral("提示"), QStringLiteral("信件内容不能为空！"));
        return;
    }
...
void IMMailWriteWidget::initIMMailWriteWidget()
{
    ...
    // 左侧标签部分
    QLabel *labelSender = new QLabel(QStringLiteral("发件人:"), this);
    QLabel *labelReceiver = new QLabel(QStringLiteral("收件人:"), this);
    QLabel *labelTheme = new QLabel(QStringLiteral("主    题:"), this);
    ...
    // 右侧编辑部分
    QLabel *senderAddress = new QLabel(QString(QStringLiteral("%1%2"))
                                       .arg(m_myID, MAIL_ADDRESS_FORMAT), this);
    m_leReceiverAddress = new QLineEdit(this);
    m_leReceiverAddress->setPlaceholderText(QStringLiteral("请输入对方地址"));
...
//    m_leReceiverAddress->setText(MAIL_ADDRSS_FORMAT);
    ...
    m_leTheme->setPlaceholderText(QStringLiteral("请输入主题"));
...
//    topLayout->setContentsMargins(0,0,0,0);
    ...
    // 底部按钮部分
    m_btnSend = new QPushButton(QStringLiteral("发送"), this);
