...
void IMFileSenderWidget::showRefused()
{
    m_isStopped = true;
    m_fileSender->close();
    m_labelFileStatus->setText(QStringLiteral("对方拒绝接收！"));
}
...
bool IMFileSenderWidget::startSending(const QString & file)
{
    ...
    m_labelFileName->setText(QString(QStringLiteral("文件名: %1")).arg(m_fileName));
    // 开始监听
    if (!m_fileSender->listen(QHostAddress::Any, m_tcpPort))
    {
        qDebug() << "file server error: "<< m_fileSender->errorString();
        QMessageBox::critical(NULL, QStringLiteral("IM FileSender"),
                              QStringLiteral("Unable to start file server: %1")
...
//        exit(EXIT_FAILURE);
    ...
    m_labelFileStatus->setText(QStringLiteral("等待对方接受"));
    return true;
}
...
{
    m_labelFileName = new QLabel(QStringLiteral("文件名:"), this);
    m_labelFileStatus = new QLabel(QStringLiteral("等待对方接受"), this);
...
void IMFileSenderWidget::sendMessage()
{
    ...
    m_labelFileStatus->setText(QStringLiteral("开始传送文件！"));

    m_localFile = new QFile(m_filePath);

    if(!m_localFile->open((QFile::ReadOnly)))
    {//以只读方式打开
        QMessageBox::warning(this,QStringLiteral("应用程序"),QStringLiteral("无法读取文件:\n%2").arg(m_filePath).arg(m_localFile->errorString()));
        return;
    }
...
void IMFileSenderWidget::updateClientProgress(qint64 numBytes)
{
   ...
   m_labelFileStatus->setText(QStringLiteral("已发送 %1MB (%2MB/s) \n共%3MB 已用时:%4秒\n估计剩余时间：%5秒")
    ...
    {
        m_isStopped = true;
        m_labelFileStatus->setText(QStringLiteral("传送文件成功"));
    }
}
...
{
    m_isStopped = true;
    m_labelFileStatus->setText(QStringLiteral("文件传送被中断！"));
