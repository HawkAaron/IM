    ...
    : QWidget(parent)
{
    initIMLoginWidget();
    linkSignalWithSlot();
    setWindowTitle(QStringLiteral("登录IM"));
}
                                   ...
                                   const UserInformation *me)
{
    ...
    if (isLogin)
    {
        ...
        m_btnLogin->setText(QStringLiteral("取消"));
        m_labelStatus->setText(QStringLiteral("%1").arg(mes));
...
//        }
    ...
    {
        m_isLogin = true;
        m_btnLogin->setText(QStringLiteral("登录"));
...
//        IMClientMessageCtrl::sm_hostPort = port;
        ...
        m_labelStatus->setText(QStringLiteral("登录中"));
        ...
        QString pwd = m_leUserPwd->text(); // IMEncryption::getInstace().getXorEncryptDecrypt(m_leUserPwd->text(), 10);
        qDebug() << "ID: " << m_leUserID->text() << " Password: " << pwd;
    ...
    else
    {
        m_isLogin = true;
        m_btnLogin->setText(QStringLiteral("登录"));
        m_labelStatus->setText(QStringLiteral("你好,请登录系统."));
...
//    forgotPwdWidget->show();
    //delete this;
    bool isOkMes = false;
    QString userID = QInputDialog::getText(NULL, QStringLiteral("找回密码"),
                                           QStringLiteral("请输入帐号ID"),
                                           QLineEdit::Normal,
                                           NULL,
                                           &isOkMes);
    if (!isOkMes)
...
void IMLoginWidget::initIMLoginWidget()
{
    ...
    QLabel *labelHeadLine = new QLabel(QStringLiteral("登录即时通信系统"));
...
//    QLabel *labelID = new QLabel(QStringLiteral("ID"));
//    QLabel *labelPwd = new QLabel(QStringLiteral("Password"));
//    vLayoutLeftTop->addWidget(labelID);
//    vLayoutLeftTop->addWidget(labelPwd);

    m_leUserID = new QLineEdit;
    m_leUserID->setPlaceholderText(QStringLiteral("帐号"));
    m_leUserPwd = new QLineEdit;
    m_leUserPwd->setPlaceholderText(QStringLiteral("密码"));
    ...
    m_labelRegister->setText(QStringLiteral("注册帐号"));
    m_labelForgotPwd = new IMClickLabel;
    m_labelForgotPwd->setText(QStringLiteral("忘记密码"));
...
//    hLayoutTop->addLayout(vLayoutLeftTop);
    ...
    //mid
    m_cbKeepPwd = new QCheckBox(QStringLiteral("记住密码"));
    m_cbAutoLogin = new QCheckBox(QStringLiteral("自动登录"));
    hLayoutMid->addWidget(m_cbKeepPwd);
    hLayoutMid->addWidget(m_cbAutoLogin);
    //end mid

    //bottom
    m_moreButton = new QPushButton(QStringLiteral("设置"));
    m_moreButton->setCheckable(true);
    m_moreButton->setAutoDefault(false);
    m_cbxStatus = new QComboBox();
    m_cbxStatus->addItem(QStringLiteral("在线"));
    m_cbxStatus->addItem(QStringLiteral("隐身"));
    m_cbxStatus->addItem(QStringLiteral("忙碌"));
    m_cbxStatus->addItem(QStringLiteral("离开"));
    m_cbxStatus->addItem(QStringLiteral("请勿打扰"));
    ...
    m_hostAddress->setPlaceholderText(QStringLiteral("服务器IP地址"));
    m_hostAddress->setText("115.159.49.46");
    m_hostPort = new QLineEdit;
    m_hostPort->setFixedWidth(60);
    m_hostPort->setPlaceholderText(QStringLiteral("端口号"));
