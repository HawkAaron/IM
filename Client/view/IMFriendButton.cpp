...
void IMFriendButton::onClickRemoveFriend()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("删除好友"),
        QString(QStringLiteral("你确定要删除好友(%1)吗？")).arg(m_info.m_userID),
...
void IMFriendButton::creatMenu(/*const QList<QString> *groutList*/)
{
    ...
    QAction *sendMessage = new QAction(QStringLiteral("发送即时消息"), m_menu);
    QAction *showRecord = new QAction(QStringLiteral("查看消息记录"), m_menu);
    QAction *sendMail = new QAction(QStringLiteral("发送电子邮件"), m_menu);

    QAction *showInfo = new QAction(QStringLiteral("查看资料"), m_menu);
    QAction *changeRemar = new QAction(QStringLiteral("修改备注名称"), m_menu);
    QAction *removeFriend = new QAction(QStringLiteral("删除好友"), m_menu);
    QAction *enterSpace = new QAction(QStringLiteral("进入好友空间"), m_menu);

    m_moveFriendMenu= new QMenu(QStringLiteral("移动好友至"), m_menu);
    refreshMoveMenu();
