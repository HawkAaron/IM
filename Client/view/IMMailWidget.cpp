    ...
    : m_myID(myID), QWidget(parent)
{
    ...
    initIMMailWidget();
    linkSignalWithSlot();
    setWindowTitle(QStringLiteral("IM邮件"));
}
...
void IMMailWidget::showMailInformationWidget(const MailInformation & mail)
{
    ...
    if (count < 4)
    {
        m_tabWidget->addTab(m_informationWidget, QStringLiteral("信件详情"));
    }
...
void IMMailWidget::initIMMailWidget()
{
    ...
    m_tabWidget->addTab(m_inWidget, QStringLiteral("收件箱"));
    m_tabWidget->addTab(m_outWidget, QStringLiteral("发件箱"));
    m_tabWidget->addTab(m_writeWidget, QStringLiteral("写信"));
//    m_tabWidget->setTabPosition(QTabWidget::West);
    m_labelInformation = new QLabel(QString(QStringLiteral("%1, 你好！")).arg(m_myID), this);
