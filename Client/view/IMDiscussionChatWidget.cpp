    ...
    QWidget(parent)
{
    ...
    setLabelStatus();
    linkSignalWithSlot();
    setWindowTitle(QStringLiteral("IM组聊窗口"));
}
...
void IMDiscussionChatWidget::onClickBtnHistory(bool checked)
{
    if (checked)
    {
        m_labelDockWidget->setText(QStringLiteral("消息记录"));
        readHistoryMessage();
    }
    else
        m_labelDockWidget->setText(QStringLiteral("讨论组成员列表"));
...
void IMDiscussionChatWidget::onClickBtnSend()
{
    ...
    QString temp = QString("<font size=\"FONT_SIEZE\" color=green>%1    %2: </font>%3")
        .arg(QStringLiteral("我")).arg(dateTime).arg(sendString);
    m_tbMessageShow->append(temp);
...
void IMDiscussionChatWidget::initIMDiscussionChatWidget()
{
    ...
    m_labelDockWidget->setText(QStringLiteral("群成员列表"));
