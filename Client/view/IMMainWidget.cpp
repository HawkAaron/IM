...
void IMMainWidget::addFriend(const QString friendID)
{
    ...
    {
        QMessageBox::critical(NULL, QStringLiteral("添加失败"), QStringLiteral("不能添加自己"));
        return;
    }
    if (m_friendMap.contains(friendID))
    {
        QMessageBox::critical(NULL, QStringLiteral("添加失败"), QStringLiteral("该用户已经是您的好友"));
        return;
    }

    bool isOkMes = false;
    QString checkMes = QInputDialog::getText(NULL, QStringLiteral("添加好友"),
                                           QStringLiteral("请输入验证信息"),
                                           QLineEdit::Normal,
                                           QStringLiteral("你好,我是: %1").
...
{
    m_onlineCounter = num;
    m_labelOnlineNum->setText(QString(QStringLiteral("在线人数: %1"))
...
{
    QString csv = QStringLiteral("CSV文件 (*.csv)");
    QString excel = QStringLiteral("EXCEL文件 (*.xls)");
    QString txt = QStringLiteral("TXT文件 (*.txt)");
    ...
    QString filter = QString(QStringLiteral("%1;;%2;;%3").arg(csv, excel, txt));
    QString name = QFileDialog::getSaveFileName(this,QStringLiteral("保存文件"), path,
                                                filter, &selFilter);
    if(name.isEmpty())
    {
        return;
    ...
    if (type < 0)
    {
        temp = QStringLiteral("聊天记录导出失败！");

    }
    else if (type == 0)
    {
        temp = QStringLiteral("聊天记录为空！");
    }
    else
    {
        temp = QStringLiteral("聊天记录导出成功:%1.").arg(name);
...
//    TMP_LOCAL_HISTORY_PARH.append(QTime::currentTime().toString("hh-mm-ss"));
    ...
    if (type < 0)
    {
        temp = QString(QStringLiteral("聊天记录导出失败！"));

    }
    else if (type == 0)
    {
        temp = QString(QStringLiteral("聊天记录为空！"));
    }
    else
    {
        temp = QString(QStringLiteral("聊天记录导出成功:%1.")).arg(name);
        copyFileToPath(newPath, name, true);
...
void IMMainWidget::deleteLocalMessage(const int index)
{
    ...
    if (type < 0)
    {
        temp = QString(QStringLiteral("删除聊天记录失败！"));
    }
    else if (type == 0)
    {
        temp = QString(QStringLiteral("聊天记录为空！"));
    ...
    {
        temp = QString(QStringLiteral("删除聊天记录成功！"));
    }
    QMessageBox::information(NULL, QStringLiteral("删除聊天记录"), temp);
    m_messageManWidget->raise();
    return;
}
...
////                QStringLiteral("CSV文件 (*.xls);;EXCEL文件 (*.xls);; TXT文件 (*.txt)"));
////    QString name = QFileDialog::getSaveFileName(this,QStringLiteral("保存文件"), path,
////                                                filter, &selFilter);
////    if(name.isEmpty())
////    {
...
//    if (type < 0)
//    {
//        QString temp = QString(QStringLiteral("聊天记录导出失败！"));
//        QMessageBox::information(NULL, QStringLiteral("聊天记录"), temp);
//        return;
//    }
////    qDebug() << "type: "<< type;
////    if (type >=0)
...
////        file.close();
////    }
//    QString temp = QString(QStringLiteral("聊天记录导出成功:%1。\n请选择保存路径。")).arg(newPath);
//    QMessageBox::information(NULL, QStringLiteral("聊天记录"), temp);
...
//            m_flockRoomMap[mes.m_receiverID]->show();
        ...
        if (!m_flockMap[mes.m_receiverID]->isRoomShow())
        {
            ...
            if (!m_linkMap.contains(mes.m_receiverID))
            {
                ...
                m_tabWidget->setTabText(3, QString(QStringLiteral("新消息%1"))
...
void IMMainWidget::dropedFlock(const QString & flockID)
{
    ...
    QMessageBox::information(NULL, QStringLiteral("群消息"),
                             QString(QStringLiteral("群%1已经被群主解散")).arg(flockID));
...

{
    ...
    QMessageBox::information(NULL, QStringLiteral("讨论组消息"),
                             QString(QStringLiteral("讨论组%1已经被组长解散")).arg(discussionID));
                               ...
                               const QString & creatorID)
{
    ...
    {
        QMessageBox::information(NULL, QStringLiteral("群消息"),
                                 QString(QStringLiteral("用户%1已经退出群%2")).
    ...
    {
        QMessageBox::information(NULL, QStringLiteral("群消息"),
                                 QString(QStringLiteral("您好,%1,您已经退出群%2")).
                                 arg(memberID, flockID));
                                    ...
                                    const QString & discussionID)
{
    ...
    {
//        QMessageBox::information(NULL, QStringLiteral("讨论组消息"),
//                                 QString(QStringLiteral("您好,%1,您已经退出讨论组%2")).
...
//         if (creatorID == m_myself.m_userID)
    ...
    {
//             QMessageBox::information(NULL, QStringLiteral("群消息"),
//                                      QString(QStringLiteral("用户%1已经退出讨论组%2")).
//                                      arg(memberID, discussionID));
...
void IMMainWidget::createDiscussionSuccess(const DiscussionInformation & discussion)
{
    ...
    QMessageBox::information(NULL, QStringLiteral("讨论组消息"), temp);
}
...
{
    bool isOk = false;
    QString friendID = QInputDialog::getText(NULL, QStringLiteral("添加好友"),
                                           QStringLiteral("请输入对方帐号"),
                                           QLineEdit::Normal,
                                           "",
                                           &isOk);
    if (!isOk)
...
{
    bool isOk = false;
    QString flockID = QInputDialog::getText(NULL, QStringLiteral("添加群"),
                                           QStringLiteral("请输入群帐号"),
                                           QLineEdit::Normal,
                                           "",
                                           &isOk);
    if (!isOk)
    ...
    if (m_flockMap.contains(flockID))
    {
        QMessageBox::critical(NULL, QStringLiteral("添加失败"), QStringLiteral("你已经加入该群"));
        return;
    }

    bool isOkMes = false;
    QString checkMes = QInputDialog::getText(NULL, QStringLiteral("添加群"),
                                           QStringLiteral("请输入验证信息"),
                                           QLineEdit::Normal,
                                           QStringLiteral("你好,我是: %1").
                                             arg(m_myself.m_nickname),
...
{
    bool isOkMes = false;
    QString oldPwd = QInputDialog::getText(NULL, QStringLiteral("修改密码"),
                                           QStringLiteral("请输入原密码:"),
                                           QLineEdit::Password,
                                           NULL,
                                           &isOkMes);
    if (!isOkMes)
    ...
    while(1)
    {
        QString newPwdOne = QInputDialog::getText(NULL, QStringLiteral("修改密码"),
        ...
        {
            QMessageBox::critical(NULL, QStringLiteral("修改密码"),
                                  QStringLiteral("新密码长度不能少于6个字符！"));
            continue;
        }
        if (newPwdOne.length() > 14)
        {
            QMessageBox::critical(NULL, QStringLiteral("修改密码"),
                                  QStringLiteral("新密码长度不能超过14个字符！"));
            continue;
        }

        QString newPwdTwo = QInputDialog::getText(NULL, QStringLiteral("修改密码"),
        ...
        {
            QMessageBox::critical(NULL, QStringLiteral("修改密码"), QStringLiteral("两次新密码不一致！"));
            continue;
        }
        else
        {
            if (NULL != m_mainCtrl)
            {
                m_mainCtrl->requestChangePwd(oldPwd, newPwdOne);
                break;
            }
        }
...
{
    bool isOk = false;
    QString flockName = QInputDialog::getText(NULL, QStringLiteral("创建群"),
                                           QStringLiteral("请输入群名"),
                                           QLineEdit::Normal,
                                           QStringLiteral("%1的IM群").
                                              arg(m_myself.m_nickname),
                                           &isOk);
    if (!isOk)
        return;
    ...
    isOk = false;
    QString flockTheme = QInputDialog::getText(NULL, QStringLiteral("创建群"),
                                           QStringLiteral("请输入群主题"),
                                           QLineEdit::Normal,
                                           QStringLiteral("没有主题"),
                                           &isOk);
    if (!isOk)
...
{
    bool isOk = false;
    QString name = QInputDialog::getText(NULL, QStringLiteral("创建讨论组"),
                                           QStringLiteral("请输入组名"),
                                           QLineEdit::Normal,
                                           QStringLiteral("%1的IM讨论组").
                                              arg(m_myself.m_nickname),
                                           &isOk);
    if (!isOk)
        return;
    ...
    isOk = false;
    QString theme = QInputDialog::getText(NULL, QStringLiteral("创建讨论组"),
                                           QStringLiteral("请输入主题"),
                                           QLineEdit::Normal,
                                           QStringLiteral("没有主题"),
                                           &isOk);
    if (!isOk)
...
void IMMainWidget::setTimerStatus(bool isOpen)
{
    if (isOpen)
    {
        if (0 == m_timerId)
        {
            ...
            m_toolBtnNewMes->setStatusTip(QStringLiteral("查阅最新消息"));
            m_toolBtnNewMes->setToolTip(QStringLiteral("有新消息"));
//                                        .arg(m_messageListWidget->size()));
        }
    }
    ...
    else if (!isOpen && m_timerId != 0)
    {
        ...
        m_timerId = 0;
        m_toolBtnNewMes->setToolTip(QStringLiteral("无新消息"));
        m_toolBtnNewMes->setStatusTip(QStringLiteral("无最新消息"));
...
{
    bool isOk = false;
    QString groupName = QInputDialog::getText(NULL, QStringLiteral("新建组"),
                                           QStringLiteral("请输入分组名"),
                                           QLineEdit::Normal,
    ...
    if (isOk)
    {
        if (m_indexFriendsGroupMap.contains(groupName))
        {
            QMessageBox::information(NULL, QStringLiteral("新建组"), QStringLiteral("无法创建，因为存在同名的组！"));
            return;
        }
...
void IMMainWidget::removeLinkButton(const QString & id)
{
    if (m_linkMap.contains(id))
    {
        ...
        m_linkMap.remove(id);
        m_tabWidget->setTabText(3, QStringLiteral("新消息(%1)")
                                .arg(m_messageListWidget->size()));
    }
    else
    {
...
void IMMainWidget::removeAllLinkButton()
{
    ...
    m_tabWidget->setTabText(3, QStringLiteral("新消息(%1)").arg(0));
}
                                        ...
                                        const UserInformation & friendInf)
{
    ...
    case REQUEST_FRIEND:
    {
        QMessageBox::StandardButton rb = QMessageBox::question(
                    NULL, QStringLiteral("好友请求"),
                    QStringLiteral("是否同意用户%1(%2)添加你为好友?\n验证消息:\n%3").
...
//        fri.m_groupName = QStringLiteral("New Friends");
        fri.m_status = friendInf.m_status;
        if (!addFriendButton(fri))
            return;
        QString temp = QStringLiteral("成功添加好友%1(%2).").
                arg(friendInf.m_userID, friendInf.m_nickname);
        QMessageBox::information(NULL, QStringLiteral("消息"), temp);
        break;
    }
    case REFUSE_FRIEND:
    {
        QString temp = QStringLiteral("用户%1(%2)\n拒绝您的好友添加请求.").
                arg(friendInf.m_userID, friendInf.m_nickname);
        QMessageBox::information(NULL, QStringLiteral("消息"), temp);
        break;
    }
                                       ...
                                       const FlockInformation &flock)
{
    ...
    {
        QMessageBox::StandardButton rb = QMessageBox::question(
                    NULL, QStringLiteral("用户请求"),
                    QStringLiteral("是否同意用户%1加入群%2?\n验证消息:\n%3").
    ...
    case AGREE_FLOCK:
    {
        ...
        QString temp = QString(QStringLiteral("用户%1,成功加入群%2(%3).")).
...
//        out << temp.m_message << m_save.m_flockMember;
    ...
    case REFUSE_FLOCK:
    {
        QString temp = QString(QStringLiteral("您好,%1,用户%2,拒绝加入群%3\n的群.")).
    ...
    case FLOCK_AGREE:
    {
        ...
        if (!addFlockButton(flock))
            return;
        QString temp = QString(QStringLiteral("成功加入群%1(%2).")).
    ...
    case FLOCK_REFUSE:
    {
        QString temp = QString(QStringLiteral("您好,%1,群%2\n的群主拒绝您的加入请求.")).
    ...
    {
        QMessageBox::StandardButton rb = QMessageBox::question(
                    NULL, QStringLiteral("群请求"),
                    QString(QStringLiteral("是否同意加入群%1?\n验证消息:\n%2")).
                                            ...
                                            const DiscussionInformation & discussion)
{
    ...
    {
        QString temp = QString(QStringLiteral("您好,%1,用户%2加入讨论组%3失败.")).
...
void IMMainWidget::removeBox(const QString & title)
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("删除分组"),
        QString(QStringLiteral("你确定要删除分组(%1)吗？")).arg(title),
    ...
    {
        QMessageBox::information(NULL, QStringLiteral("删除组"), QStringLiteral("无法删除，因为该组里还有组员！"));
        return;
    }
...
{
    bool isOk = false;
    QString newTitle = QInputDialog::getText(NULL, QStringLiteral("重命名分组"),
                                           QStringLiteral("请输入新的分组名"),
                                           QLineEdit::Normal,
                                           title,
                                           &isOk);
    if (!isOk)
    ...
    {
        QMessageBox::information(NULL, QStringLiteral("重命名分组"), QStringLiteral("失败，因为存在同名的组！"));
        return;
    }
...
//                              "QComboBox::drop-down{border-style:none;border:0px;width:40px}");
    ...
    NOTDISTURB = 4,// 请勿打扰
    */
    m_cbStatus->addItem(QStringLiteral("在线"));
    m_cbStatus->addItem(QStringLiteral("隐身"));
    m_cbStatus->addItem(QStringLiteral("忙碌"));
    m_cbStatus->addItem(QStringLiteral("离开"));
    m_cbStatus->addItem(QStringLiteral("请勿打扰"));
    ...
    m_labelMark = new QLabel(QStringLiteral("我的签名")); // 说说 签名

//    m_toolBtnSpace = new QToolButton(this);  // 空间
//    m_toolBtnSpace->setIcon(QIcon(":/images/space.png"));
//    m_toolBtnSpace->setAutoRaise(true);
//    m_toolBtnSpace->setToolTip(QStringLiteral("空间"));
//    m_toolBtnSpace->setStatusTip(QStringLiteral("打开空间"));
...
//    m_toolBtnSpace->setAutoFillBackground(true);
    ...
    m_toolBtnMail->setToolTip(QStringLiteral("邮箱"));
    m_toolBtnMail->setStatusTip(QStringLiteral("打开邮箱"));
    ...
    m_toolBtnMesBox->setToolTip(QStringLiteral("消息管理"));
    m_toolBtnMesBox->setStatusTip(QStringLiteral("打开消息管理器"));
    ...
    m_leSearch->setPlaceholderText(QStringLiteral("search friends"));
...
//    m_toolBtnSearch->setToolTip(QStringLiteral("搜索好友"));
//    m_toolBtnSearch->setStatusTip(QStringLiteral("搜索好友"));
...
//    m_tabWidget->setTabShape(QTabWidget::Triangular);
    ...
    m_tabWidget->addTab(m_friendListWidget, QStringLiteral("联系人"));
    m_tabWidget->addTab(m_flockListWidget,QStringLiteral("IM群"));
    m_tabWidget->addTab(m_discussionListWidget, QStringLiteral("讨论组"));
    m_tabWidget->addTab(m_messageListWidget, QStringLiteral("新消息(0)"));
    ...
    m_toolBtnAddFlock->setToolTip(QStringLiteral("加群"));
    m_toolBtnAddFlock->setStatusTip(QStringLiteral("添加群"));
...
//    m_toolBtnAddDiscussion->setToolTip(QStringLiteral(""));
//    m_toolBtnAddDiscussion->setStatusTip(QStringLiteral(""));
...
//    m_toolBtnSearchAdd->setToolTip(QStringLiteral(""));
//    m_toolBtnSearchAdd->setStatusTip(QStringLiteral(""));

    m_toolBtnConfig = new QToolButton;
    m_toolBtnConfig->setIcon(QIcon(":/images/config.png"));
    m_toolBtnConfig->setAutoRaise(true);
    m_toolBtnConfig->setToolTip(QStringLiteral("系统管理"));
    m_toolBtnConfig->setStatusTip(QStringLiteral("打开系统管理"));
    ...
    m_toolBtnAddFriend->setToolTip(QStringLiteral("加好友"));
    m_toolBtnAddFriend->setStatusTip(QStringLiteral("添加好友"));
...
void IMMainWidget:: creatMenu()
{
    ...
    QAction *addFriend = new QAction(QStringLiteral("添加好友"), m_menu);
    QAction *addFlock = new QAction(QStringLiteral("添加群"), m_menu);
    QAction *changePwd = new QAction(QStringLiteral("修改密码"), m_menu);
    QAction *information = new QAction(QStringLiteral("个人信息"), m_menu);
    QAction *manMes = new QAction(QStringLiteral("消息管理"), m_menu);
    QAction *mail = new QAction(QStringLiteral("邮件管理"), m_menu);
...
bool IMMainWidget::continueToClose()
{
    ...
    {
        QString temp = QStringLiteral("还有聊天窗口没有关闭，您确定想要退出程序？");
        QMessageBox::StandardButton rb;
        rb = QMessageBox::question(this, QStringLiteral("退出程序"), temp,
            QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
        if(rb == QMessageBox::Yes)
                return true;
        else
