...
void IMMessageWidget::setShowMode(Mode mode)
{
    if (mode == LOCAL)
    {
        m_labelInformation->setText(QStringLiteral("本地消息记录管理"));
        m_tooBtnImport->setVisible(true);
    }
    else if (mode == NETWORK)
    {
        m_labelInformation->setText(QStringLiteral("网络消息记录管理"));
        m_tooBtnImport->setVisible(false);
        m_toolBtnExportAll->setVisible(false);
        m_toolBtnExportAll->setVisible(false);
        m_toolBtnDeleteOne->setVisible(false);
        m_toolBtnExportOne->setToolTip(QStringLiteral("导出当前显示的消息"));
        m_toolBtnExportOne->setStatusTip(QStringLiteral("导出当前显示的消息记录"));
...
{
    /*********************顶部**************************/
    m_labelInformation = new QLabel(QStringLiteral("消息管理"),this);
    ...
    m_toolBtnGetMessage->setToolTip(QStringLiteral("获取消息"));
    m_toolBtnGetMessage->setStatusTip(QStringLiteral("获取与当前好友当前日期的消息记录"));

    m_leBirthday = new CustomLineEdit(this);
    m_leBirthday->setFixedWidth(100);
    m_leBirthday->setToolTip(QStringLiteral("选择查看日期"));
    m_leBirthday->setStatusTip(QStringLiteral("选择需要查看的日期"));

    m_tooBtnImport = new QToolButton(this);
    m_tooBtnImport->setIcon(QIcon(":/images/importAll.png"));
    m_tooBtnImport->setAutoRaise(true);
    m_tooBtnImport->setToolTip(QStringLiteral("导入消息"));
    m_tooBtnImport->setStatusTip(QStringLiteral("导入聊天记录"));
    ...
    m_toolBtnExportOne->setToolTip(QStringLiteral("导出当前好友消息"));
    m_toolBtnExportOne->setStatusTip(QStringLiteral("导出当前与好友的消息记录"));
    ...
    m_toolBtnExportAll->setToolTip(QStringLiteral("导出所有消息"));
    m_toolBtnExportAll->setStatusTip(QStringLiteral("导出所有消息记录"));
...
//    m_toolBtnDeleteDate->setToolTip(QStringLiteral("删除当前显示的消息记录"));
//    m_toolBtnDeleteDate->setStatusTip(QStringLiteral("删除当先显示的消息记录"));
    ...
    m_toolBtnDeleteOne->setToolTip(QStringLiteral("删除当前好友消息记录"));
    m_toolBtnDeleteOne->setStatusTip(QStringLiteral("删除与当前好友的消息记录"));
    ...
    m_toolBtnDeleteAll->setToolTip(QStringLiteral("删除所有消息记录"));
    m_toolBtnDeleteAll->setStatusTip(QStringLiteral("删除所有的消息记录"));
...
//    m_modelGroup->setStringList();
    ...
    m_listViewGroup->setToolTip(QStringLiteral("分组列表"));
    m_listViewGroup->setStatusTip(QStringLiteral("请选择分组"));
...
//    m_modelFriend->setStringList();
    ...
    m_listViewFriend->setToolTip(QStringLiteral("好友列表"));
    m_listViewFriend->setStatusTip(QStringLiteral("请选择好友"));
