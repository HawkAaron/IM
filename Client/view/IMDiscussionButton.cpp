...
void IMDiscussionButton::onClickDropDiscussion()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("解散讨论组"),
        QString(QStringLiteral("你确定要解散讨论组(%1)吗？")).arg(m_discussionInfo.m_discussionName),
...
void IMDiscussionButton::onClickLeaveDiscussion()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("讨论组"),
        QString(QStringLiteral("你确定要退出讨论组(%1)吗？")).arg(m_discussionInfo.m_discussionName),
        QMessageBox::Yes | QMessageBox::No,  QMessageBox::No);
...
void IMDiscussionButton::onClickAddMember()
{
    ...
    for (int i =0; i<m_discussionMembersVec.size(); ++i)
    {
        if (0 == memberID.compare(m_discussionMembersVec[i].m_userID))
        {
            QMessageBox::critical(NULL, QStringLiteral("添加失败"), QStringLiteral("该用户已经是该讨论组的成员"));
            return;
        }
    }
...
void IMDiscussionButton::creatMenu(/*const QList<QString> *groutList*/)
{
    ...
    QAction *sendMessage = new QAction(QStringLiteral("发送讨论组消息"), m_menu);
    QAction *showRecord = new QAction(QStringLiteral("查看消息记录"), m_menu);
    QAction *showInformation = new QAction(QStringLiteral("查看资料"), m_menu);
    QAction *addMember = new QAction(QStringLiteral("添加讨论组成员"), m_menu);
    ...
    {
        QAction *dropDiscussion = new QAction(QStringLiteral("解散该讨论组"), m_menu);
    ...
    {
        QAction *leaveDiscussion = new QAction(QStringLiteral("退出该讨论组"), m_menu);
