...
void IMDiscussionMemberButton::onClickRemoveMember()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("移除讨论组成员"),
        QString(QStringLiteral("你确定要将成员%1,从讨论组%2中移除吗？")).
...
void IMDiscussionMemberButton::creatMenu(/*const QList<QString> *groutList*/)
{
    ...
    QAction *enterSpace = new QAction(QStringLiteral("进入空间"), m_menu);
    QAction *removeMember= new QAction(QStringLiteral("移除该成员"), m_menu);
//    QAction *changeRemark = new QAction(QStringLiteral("修改群名片"), m_menu);
    QAction *addRriend= new QAction(QStringLiteral("加为好友"), m_menu);
