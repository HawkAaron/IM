...
void IMFlockButton::onClickDropFlock()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("解散群"),
        QString(QStringLiteral("你确定要解散群(%1)吗？")).arg(m_flockInfo.m_flockName),
...
void IMFlockButton::onClickLeaveFlock()
{
    ...
    returnBtn = QMessageBox::question(NULL, QStringLiteral("退出群"),
        QString(QStringLiteral("你确定要退出群(%1)吗？")).arg(m_flockInfo.m_flockName),
        QMessageBox::Yes | QMessageBox::No,  QMessageBox::No);
...
void IMFlockButton::onClickAddMemberFlock()
{
    ...
    for (int i =0; i<m_flockMembersVec.size(); ++i)
    {
        if (0 == memberID.compare(m_flockMembersVec[i].m_userID))
        {
            QMessageBox::critical(NULL, QStringLiteral("添加失败"), QStringLiteral("该用户已经是该群成员"));
            return;
        }
    }
    ...
    QString checkMes = QInputDialog::getText(NULL, "添加群成员",
                       "请输入验证信息",
                       QLineEdit::Normal,
                       QString(QStringLiteral("您好,我是群%1(%2)的群主，请求您加入我们的群")).
...
void IMFlockButton::creatMenu(/*const QList<QString> *groutList*/)
{
    ...
    QAction *sendMessage = new QAction(QStringLiteral("发送群消息"), m_menu);
    QAction *showRecord = new QAction(QStringLiteral("查看消息记录"), m_menu);
    QAction *showInfo = new QAction(QStringLiteral("查看资料"), m_menu);
    QAction *enterSpace = new QAction(QStringLiteral("进入群空间"), m_menu);
    // mark
    // 添加 群成员
    // 修改 群资料 放在查看资料里面
    // 删除 群成员
    ...
    {
        QAction *dropFlock = new QAction(QStringLiteral("解散该群"), m_menu);
        connect(dropFlock, SIGNAL(triggered()),
                this, SLOT(onClickDropFlock()));
        m_menu->addAction(dropFlock);

        QAction *addMember = new QAction(QStringLiteral("添加群成员"), m_menu);
    ...
    {
        QAction *leaveFlock = new QAction(QStringLiteral("退出该群"), m_menu);
