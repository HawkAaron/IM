    ...
    QWidget(parent)
{
    ...
    setWindowTitle(QStringLiteral("个人信息"));
}
...
//    m_leBirthday->setReadOnly(isReadOnly);
    ...
    if (isEdit)
    {
        if (isReadOnly)
        {
            m_btnCancel->setText(QStringLiteral("关闭"));
            m_btnOk->setText(QStringLiteral("编辑"));
        }
        else
        {
            m_btnCancel->setText(QStringLiteral("取消"));
            m_btnOk->setText(QStringLiteral("保存"));
        }
    }
    else
    {
...
void IMInformationWidget::setUserInformation(const UserInformation &user)
{
    ...
    if (0 == m_userInf.m_sex.compare(QStringLiteral("男")))
    ...
    QString labelStr = QStringLiteral("头像: %1 / %2").arg(QString::number(m_headNum)).
    ...
    if (m_isEdit)
        ...
        m_labelHead->setText(QStringLiteral("头像"));
}
...
void IMInformationWidget::initIMLoginWidget()
{
    ...
    // 左边标签
    m_labelUserID = new QLabel(QStringLiteral("帐号:"));
    ...
    m_labelName = new QLabel(QStringLiteral("姓名:"));
    ...
    // 右边 按钮 以及 头像
    m_btnNext = new QPushButton(QStringLiteral("下一张"));
    m_btnPre = new QPushButton(QStringLiteral("上一张"));
    QHBoxLayout *hLayoutBtnEdit = new QHBoxLayout;
    hLayoutBtnEdit->addWidget(m_btnPre);
    hLayoutBtnEdit->addWidget(m_btnNext);
    m_btnOk = new QPushButton(QStringLiteral("编辑"));
    ...
    m_labelHead = new QLabel(QStringLiteral("头像"));
...
//    m_frameHead->setStyleSheet(str);
    ...
    QString str = QString("QFrame#avatar{border-image:url(resource/image/head/1.bmp);}");
    m_frameHead->setStyleSheet(str);
    m_labelHead->setText(QStringLiteral("头像: 1 / 133"));
}
