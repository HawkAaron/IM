...
void IMLoginCtrl::readMessage()
{
    ...
    case LOGIN_SUCCESS:
        in >> m_myself;
        emit getLoginMessgae(QStringLiteral("登录成功"), true, &m_myself);
        break;
    case LOGIN_FAIL:
        emit getLoginMessgae(QStringLiteral("登录失败.帐号或者密码错误."),false);
        break;
    case HAVE_LOGINED:
        emit getLoginMessgae(QStringLiteral("登录失败.该用户已经登录."),false);
        break;
    case GET_QUESTION_ANSWER_FAIL:
    {
        QMessageBox::critical(NULL, QStringLiteral("找回密码"), QStringLiteral("失败，帐号不存在！"));
        break;
    }
    case GET_QUESTION_ANSWER_SUCCESS:
    {
        ...
        {
            bool isOkMes = false;
            QString str = QString(QStringLiteral("密保问题:%1\n请输入问题答案:"))
                    .arg(m_tempStr.m_two);
            ...
            {
                str = QString(QStringLiteral("回答错误!"));
                QMessageBox::critical(NULL, QStringLiteral("找回密码"), str);
                continue;
            }
            else
            {
                str = QString(QStringLiteral("回答正确!\n您的帐号是:%1\n您的密码是:%2"))
                        .arg(m_id)
                        .arg(m_tempStr.m_one);
                QMessageBox::information(NULL, QStringLiteral("找回密码"), str);
            }
            break;
        }
        break;
