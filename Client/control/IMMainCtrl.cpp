...
/*************************************************
Function Name： requestGetLatestMessage()
Description: 发送获取离线消息的请求
*************************************************/
void IMMainCtrl::requestGetLatestMessage(const QString & id)
{
    if (NULL == m_tcpSocket)
        return;
    qDebug() << "request Latest Message: " << id;
    m_blockSize = 0;
    QByteArray block;
    QDataStream out(&block, QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_4_8);
...
void IMMainCtrl::readMessage()
{
    ...
    case GET_ALL_FRIENDS_SUCCESS:
    {
        ...
        // 根据在线状态排序
        qSort(m_friendsVec.begin(), m_friendsVec.end());
    ...
    {
        QMessageBox::critical(NULL, QStringLiteral("修改个人信息"), QStringLiteral("修改个人信息失败"));
        break;
    }
    case CHANGE_INFORMATION_SUCCESS:
    {
        ...
        QMessageBox::information(NULL, QStringLiteral("修改个人信息"), QStringLiteral("修改个人信息成功"));
        break;
    }
    case TALK:
    {
    ...
    case BE_DELETED:
    {
        ...
        QString temp = QString(QStringLiteral("你被好友(%1)删除了")).arg(m_peerID);
...
//    }
    ...
    {
        QMessageBox::information(NULL, QStringLiteral("修改密码"), QStringLiteral("修改密码失败！"));
        break;
    }
...
//        }
        ...
        QMessageBox::information(NULL, QStringLiteral("修改密码"), QStringLiteral("修改成功！请重新登录"));
...
void IMMainCtrl::closeWindow()
{
    if (m_tcpSocket->flag() == 0)
    {
        ...
        QMessageBox::information(NULL, QStringLiteral("系统提示"),
                              QStringLiteral("您的客户端已经与服务器断开连接，请重新登录。"));
        emit closeWindowSignal();
        IMLoginWidget *loginWidget = new IMLoginWidget;
        loginWidget->show();
