...
void IMServerMessageCtrl::runServer()
{
   ...
   {
       qDebug("run fail");
       QMessageBox::critical(NULL, QStringLiteral("IM Server"),
   ...
   qDebug()<<QStringLiteral("The server is running on port %1.").arg(m_tcpServer->serverPort());
...
//   connect(m_tcpServer, SIGNAL(newClientSocket(IMClientSocket *)), this, SLOT(readMessage(IMClientSocket *)));
   ...
   //                          QStringLiteral("Runing to Server..."));
   //     if (m_tcpServer->isListening())
   //     {
   //         QMessageBox::information(NULL, QStringLiteral("IM Client"),
   //                                  QStringLiteral("The server is running on port %1.").arg(m_tcpServer->serverPort()));
   //     }
}
