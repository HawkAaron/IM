...
static const QString INSERT_MESSAGE_IN_OFFLINEMESSAGE =
        "insert into OfflineMessage (kind, type, senderID, receiverID, content, dateTime) "
        "values(?, ?, ?, ?, ?, ?);";
static const QString READ_MESSAGE_FROM_OFFLINEMESSAGE =
        "select kind, type, senderID, receiverID, content, dateTime from OfflineMessage "
        "where receiverID = ?";
static const QString DELETE_OFFLINEMESSAGE =
...
bool IMDatabaseCtrl::createConnection()
{
    ...
    {
        *m_db = QSqlDatabase::database("server");
    }
    else
    {// 如果没有，就添加
...
Description: 存储离线记录
*************************************************/
bool IMDatabaseCtrl::addOfflineMessage(int kind, const TalkMessage & mes)
{
    if (!createConnection())
    {
        return false;
...
/*************************************************
Function Name： readOfflineMessage()
Description: 读取离线记录
*************************************************/
bool IMDatabaseCtrl::readOfflineMessage(const QString& id, QVector<int> &mesKind, QVector<TalkMessage> & mesVec)
{
    if (!createConnection())
    {
        return false;
    ...
    QSqlQuery query(*m_db);
    query.prepare(READ_MESSAGE_FROM_OFFLINEMESSAGE);
    query.addBindValue(id);
    query.exec();

    errorSQLOrder(query, "read-mes-from-Offline-Mmesage");
    if (!query.isActive())
    {
        qDebug() << "QUERY_FAIL::" << query.lastError().text();
        m_db->close();
    ...
    while (query.next())
    {
        ...
        mes.m_dateTime = query.value(5).toDateTime();
        mesVec.push_back(mes);
    }

    QSqlDatabase::database().commit();

    delOfflineMessage(id);

    return true;
}

/*************************************************
Function Name： delOfflineMessage()
Description: 删除离线记录
*************************************************/
bool IMDatabaseCtrl::delOfflineMessage(const QString& id)
{
    if (!createConnection())
    {
        return false;
    ...
    QSqlQuery query(*m_db);
    query.prepare(DELETE_OFFLINEMESSAGE);
    query.addBindValue(id);
    query.exec();

    errorSQLOrder(query, "del-mes-from-Offline-Mmesage");
    if (!query.isActive())
    {
        qDebug() << "QUERY_FAIL::" << query.lastError().text();
        m_db->close();
