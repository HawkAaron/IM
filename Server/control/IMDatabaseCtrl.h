...
//    int createGroup(const QString & id, const QString & title);
    ...
    // 存储离线记录
    bool addOfflineMessage(int kind, const TalkMessage &);
    // 读取离线记录
    bool readOfflineMessage(const QString& id, QVector<int>& mesKind, QVector<TalkMessage> & mesVec);
    // 删除离线记录
    bool delOfflineMessage(const QString& id);
    // 获取用户的某日与某位好友的聊天记录
