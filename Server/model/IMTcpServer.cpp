...
//        QMultiMap<QString, FriendsInfo> *userMap= new QMultiMap<QString, FriendsInfo>;
        ...
        if (GET_ALL_FRIENDS_FAIL != m_save.m_replyKind)
        {
            if (!m_userMap.contains(m_save.m_myID))
            {
                ...
                emit haveNewAddress(m_save.m_clientSocket->peerAddress().toString());
            }
...
//        save.m_clientSocket->sendMessage(m_save);
    ...
    case GET_OFFLINE_MESSAGE:
    {
        ...
        m_kind.clear();
        m_offlineMes.clear();
        m_database.readOfflineMessage(m_save.m_myID, m_kind, m_offlineMes);
        for (int i=0; i<m_kind.size(); i++) {
            m_save.m_requestKind = m_kind[i];
            m_save.m_message = m_offlineMes[i];
            sendMessage(m_save);
        }
    }
...
void IMTcpServer::friendRequest(const SaveTmpInformation &save)
{
    ...
    {
        // 存储离线消息
        m_database.addOfflineMessage(ADD_FRIEND, m_save.m_message);
    }
    else
        iter.value()->sendMessage(m_save);
}
...
void IMTcpServer::talkRequest(const SaveTmpInformation & save)
{
    ...
    case TALK_MESSAGE:// == m_save.m_message.m_type)
    {
        ...
        {
            // 存储离线消息到数据库
            m_database.addOfflineMessage(TALK, m_save.m_message);
        }
        else {
            // 聊天记录存储到数据库
